...
today = date.today().strftime("%Y-%m-%d")   # e.g. "2025-08-27"
...
def normalize_timestamp(df, col="timestamp"):
    ...
    if df[col].dt.tz is not None:
        # If timezone-aware, drop tz info but keep UTC clock time
...
def tidy_weather(path: Path):
    ...
    long["source"] = "weather"
    long = long.dropna(subset=["timestamp"])  # drop NaT
    return long
...
def tidy_inverter(path: Path):
    if path.suffix.lower() == ".xlsx":
        df = pd.read_excel(path, header=2)  # header at row 3
    else:
